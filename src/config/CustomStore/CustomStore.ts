...
class CustomStore {
    ...
    public AddItem<T>(data: ICustomStoreRegister<T>): boolean {
        ...
        if (previousItem) {
            LogUtil.LogEvent('CustomStore', 'CustomStore.AddItem - Error whlie adding item: itemKey already existis on:', previousItem);
            return false;
        }

        try {
            this.Store.push(data);
            LogUtil.LogEvent('CustomStore', 'CustomStore.AddItem - Added:', data);
            return true;
        }
        catch (error) {
            LogUtil.LogEvent('CustomStore', 'CustomStore.AddItem - Error while adding item:', {error});
            return false;
        }
    }
    ...
    public GetItem<T>(key: CustomStoreKeys) {
        ...
        LogUtil.LogEvent('CustomStore', 'CustomStore.GetItem - Retrieved item:', item);
